...
labelseg <- function(data, genome ='hg38',baseshift = 'n', method = 'dbscan', len.sep = TRUE, minPts = 1, lb.dup = 0.15, hb.dup = 0.7, lb.del=0.15, hb.del = 1.5){
    # check input
    ...
    for (i in seq_len(sample.num)){
        ...
        if (len.sep){
            ...
            result[[i]] <- labelindseg(data=inddata,genome=genome,baseshift=baseshift,method=method,minPts=minPts,lb.dup=lb.dup,hb.dup=hb.dup,lb.del=lb.del,hb.del=hb.del)
        } else{
            # uniform clustering
            result[[i]] <- labelindseg_special(data=inddata,genome=genome,baseshift=baseshift,minPts=minPts,lb.dup=lb.dup,hb.dup=hb.dup,lb.del=lb.del,hb.del=hb.del)
        }
    }
